...
WP_OPAL_Registry.register("Medical-Postype", (function($) {
	"use strict";
	var state;
	...
	function createInstance() {
		return {
			init: function() {
				...
				$('#IconPickerMedical').fontIconPicker({
...
(function($) {
	...
	$(document).ready(function() {
		...
		if ('opal_medical' === window.pagenow) {
			WP_OPAL_Registry._get("Medical-Postype").init();
		}

		/**
		* --- Event onChange for image select
		*/
		if($('#medical_image_size').val() == "other"){
			$('.cmb2-id-medical-other-size').show();
		}else{
			$('.cmb2-id-medical-other-size').hide();
		}

		//--------------
		$('#medical_image_size').on('change', function(){
